...
# We need to implement SwipeToDismiss
# Swipe right to complete (green)
# Swipe left to delete (red)
# After distance and release -> trigger

new_item = '''@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun ManageTimerItem(
    task: TimerTask,
    currentTime: Long,
    ...
    val progress = if (totalDuration <= 0) 1f else if (isLapsed) 1f else ((currentTime - task.createdAt).toFloat() / totalDuration).coerceIn(0f, 1f)
...
# Replace the entire ManageTimerItem function
content = re.sub(r'@Composable\nfun ManageTimerItem\([\s\S]+?(?=@Composable\nfun CompletedHistoryTab)', new_item, content)
